# Analyzing HTTP GET Requests and TCP Packet Flow Using Wireshark
...
## Summary of Wireshark Capture and Observed Frames

After the HTTP GET request in Frame 21, the intervening frames are TCP packets handling acknowledgments and data transfer. Once TCP has delivered enough data to reconstruct the response, Wireshark displays the HTTP response in Frame 27.

### What happened between Frame 21 and Frame 27?
The frames between the two HTTP messages are TCP‑level packets, not HTTP packets. 

**Frame 21 – HTTP GET**

The client sends an HTTP GET request to the server.
This HTTP message is carried inside a TCP segment.

**Frames 22–26 – TCP communication**

These frames handle reliable data delivery, which is TCP’s job.
They typically include:

- ACK (Acknowledgment) packets, confirming receipt of data
- TCP segmentation, where the server prepares to send the response in chunks
- Flow control, ensuring neither side sends data too quickly

⚠️ Important:
Even though these frames don’t show “HTTP” in Wireshark, they are still part of the same HTTP exchange.

**Frame 27 – HTTP Response**

- The server sends the HTTP response (for example, 200 OK).
- This response is encapsulated inside TCP and reassembled by Wireshark.
- Once enough TCP segments are received, Wireshark displays the next HTTP frame.

  <img width="1717" height="720" alt="image" src="https://github.com/user-attachments/assets/948c0bf1-2f7b-4fad-8400-291e81526070" />
